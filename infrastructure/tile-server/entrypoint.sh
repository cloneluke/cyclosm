...
if [ "$1" = "tile-generation" ]; then
    ...
    if [ ! -f /data/sources/wisconsin.osm.pbf ]; then
        echo "Downloading Wisconsin..."
        curl -L https://download.geofabrik.de/north-america/us/wisconsin-latest.osm.pbf \
            -o /data/sources/wisconsin.osm.pbf
    fi
